...
vitality.2ps <- function(time = 0:(1 - length(sdata)), 
                         sdata, 
                         init.params = FALSE, 
                         rc.data = FALSE, 
                         se = FALSE, 
    ...
    fit.nlm <- nlminb(start = rsk, objective = logLikelihood.2ps, lower = c(0, -1, 0, 0),
                      upper = c(100, 100, 50, 50), xx1 = x1, xx2 = x2, NNi = Ni)
    ...
    #  --compute standard errors---
    if (se != FALSE) {
        s.e. <- stdErr.2ps(r.final, s.final, lambda.final, beta.final, x1, x2, Ni, se)  
    ...
    if (pplot != FALSE) {
        plotting.2ps(r.final, s.final, lambda.final, beta.final, mlv, time, sfract, x1, x2, Ni, pplot, Iplot, Mplot, tlab, rc.data)
    }
    # ............................................................................................
...
plotting.2ps <- function(r.final,
    ...
    if (pplot != FALSE) {
        ...
        plot(time, sfract, xlab = tlab, ylab = "survival fraction",
             ylim = c(0, 1), xlim = c(0, tmax), col = 1)
        xxx <- seq(0, tmax, length = 200)
        lines(xxx, SurvFn.2ps(xxx, r.final, s.final, lambda.final, beta.final), col = 2)
    ...
    if ( Mplot != FALSE) {
        ...
        tmax <- ext * time[len]
        xxx <- seq(0, tmax, length = 200)
        mu.i <- mu.vd1(xxx, r.final, s.final)
        mu.e <- mu.vd2(xxx, r.final, lambda.final, beta.final, 0, 0)
        mu.t <- mu.vd(xxx, r.final, s.final, lambda.final, beta.final, 0, 0)
        plot(time, mu.x, xlim = c(0, tmax), xlab = tlab, ylab = "estimated mortality rate", log = "y",
             main = "Log Mortality Data and Vitality Model Fitting")
        #plot(xxx, mu.t, xlim = c(0, tmax), xlab = tlab, ylab = "estimated mortality rate", log = "y",
        #    type = "l")
        lines(xxx, mu.t)
        lines(xxx, mu.i, col = 2, lty = 2)
        lines(xxx, mu.e, col = 3, lty = 3)
    } 
    ...
    if (Iplot != FALSE) {
        ...
        xx1 <- xxx[1:(npt-1)]
        xx2 <- xxx[2:npt]
        sProbI <- survProbInc.2ps(r.final, s.final, lambda.final, beta.final, xx1, xx2)
        
        ytop <- 1.1 * max(max(sProbI/(xx2-xx1)), Ni/(x2-x1)) * scale		
        plot((x1+x2)/2, Ni*scale/(x2-x1), ylim = c(0, ytop), xlim = c(0, ext*x2[ln]),
...
#' @return vector of FF?
SurvFn.2ps <- function(xx, r, s, lambda, beta) {
    yy <- s^2*xx
...
survProbInc.2ps <- function(r, s, lambda, beta, xx1, xx2){
    value.iSP <- -(SurvFn.2ps(xx2, r, s, lambda, beta) - SurvFn.2ps(xx1, r, s, lambda, beta))
...
logLikelihood.2ps <- function(par, xx1, xx2, NNi) {
    # --calculate incremental survival probability--- (safeguraded > 1e-18 to prevent log(0))
    iSP <-  survProbInc.2ps(par[1], par[2], par[3], par[4], xx1, xx2)
...
stdErr.2ps <- function(r, s, k, u, x1, x2, Ni, pop) {	
    LL <- function(a, b, c, d, r, s, k, u, x1, x2, Ni) {logLikelihood.2ps(c(r+a, s+b, k+c, u+d), x1, x2, Ni)}
